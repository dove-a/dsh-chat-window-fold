...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Minimum interval between top-scroll expand starts (ms). The
		 * expandingRef latch already prevents re-entry WHILE a load is in
		 * flight; this throttle additionally paces consecutive loads when the
		 * user keeps resting at the top — continuous top-scroll ticks can only
		 * start one expand per window, instead of one per scroll event.
		 */
		var EXPAND_MIN_INTERVAL_MS = 200;
		/**
		 * Max frames to wait for React to commit the prepended rows before
		 * giving up on anchoring (a correction that measured zero growth is
		 * re-attempted next frame; beyond this the latch just releases).
		 */
		var ANCHOR_MAX_TRIES = 10;
		...
		function WindowFoldDock(props) {
			...
			var lastExpandAtRef = react.useRef(0);
			...
			function expandAnchored() {
				...
				// Throttle: at most one expand start per 200ms window, so
				// resting at the top with continuous scroll ticks cannot queue
				// a load per event. (The expandingRef latch above covers the
				// in-flight case; this covers the between-loads case.)
				var now = Date.now();
				if (now - lastExpandAtRef.current < EXPAND_MIN_INTERVAL_MS) return;
				lastExpandAtRef.current = now;
				var op = typeof loadOlder === "function" ? loadOlder : null;
				if (!op) return;
				expandingRef.current = true;
				var column = columnOf(scroller);
				// Folded pages come back before newer pages load. This runs
				// BEFORE the anchor/height samples: restoring hidden rows adds
				// their height above the anchor row, so sampling later keeps
				// the anchor baseline and the height delta free of that noise.
				restoreHidden();
				var rows = rowsOf(column);
				...
				Promise.race([pending, timeout]).then(function () {
					// Anchor the viewport AFTER React has committed the prepend.
					// The loader may resolve before the window-store update
					// reaches the DOM (React schedules the commit), so a single
					// rAF can run against the OLD layout: the anchor-row delta
					// measures 0, the height delta measures 0, the correction
					// is a no-op, the viewport stays at the top — and the next
					// wheel tick starts another load. That is the observed
					// "two (or more) consecutive loads after one top-scroll".
					// Poll across frames until the layout reflects the load
					// (or ANCHOR_MAX_TRIES), then correct exactly once.
					var tries = 0;
					function attempt() {
						...
						if (anchorKey !== null && anchorTop !== null) {
							...
							if (el) {
								var delta = flowTop(el, live) - anchorTop;
								if (delta !== 0) {
						...
						if (!corrected) {
							...
							if (grown > 0) {
								live.scrollTop += grown;
								corrected = true;
							}
						}
						if (!corrected && tries < ANCHOR_MAX_TRIES) {
							// Layout has not reflected the load yet (React has
							// not committed): wait for the next frame.
							tries++;
			...
			function handleScroll() {
				...
				if (!scroller) return;
				refreshBottom();
				if (scroller.scrollTop <= TOP_EPSILON && !expandingRef.current) {
